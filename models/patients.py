...
class SdClinicPatients(models.Model):
    _name = "sd_clinic.patients"
    ...
    age = fields.Char(compute="_age_compute",)
    ...
    def _age_compute(self):
        ...
        for rec in self:
            if rec.birth_date:
                rec.age = str(datetime.now().year - rec.birth_date.year)
                # rec.age = f"{datetime.now().year - rec.birth_date.year}   {years}"
            else:
                rec.age = ''
